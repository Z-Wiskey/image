...
def get_resnet18(pretrained, output_dim, ckpt_path):
    ...
    # 只有当 ckpt_path 不为空且存在时才加载本地权重，否则直接用 ImageNet 权重
    if ckpt_path and os.path.exists(ckpt_path):
        print(f"Loading checkpoint from {ckpt_path}")
        checkpoint = torch.load(ckpt_path)
        # ... 原有的加载逻辑 ...
        checkpoint.pop('fc.weight', None)  # 使用 pop(..., None) 防止键不存在报错
...
if __name__ == "__main__":
    ...
    media_dir = "remote-view"

    train_csv_path = "train_pairs_satellite.csv"
